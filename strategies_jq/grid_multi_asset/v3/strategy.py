# -*- coding: utf-8 -*-
"""
多标的自适应网格策略 v3.0（JoinQuant 版）

移植自 SimTradeLab strategies/grid_multi_asset_v3/template.py / backtest.py。
Walk-Forward 最优参数 **Trial 357**（`best_params_20260510_232314.json`，与 SimTradeLab 一致）。

v3 相对 v2：
  - **M2** `BEAR_UNIVERSE_MODE`：BEAR + `ETF_DEFENSIVE` 时仅用 `DEFENSIVE_ETF_POOL` 选股（本参数为 **SAME**，与 v2 全池一致）
  - **M1** `BEAR_GRID_MODE`：**CAP_LAYER** + `BEAR_GRID_MAX_LAYER_CAP=0`，熊市下有效网格层上限为 0
  - **NO_NET_ADD**：依赖上一日收盘持仓市值快照 `g._prev_eod_position_value`（`after_trading_end` 写入）

PTrade → JoinQuant 映射同 `v1/strategy.py`、`v2/strategy.py` 文件头说明。
"""
import numpy as np
import pandas as pd
...
def initialize(context):
    ...
    # Walk-Forward Trial 357（与 SimTradeLab best_params_20260510_232314.json 一致）
    g.MAX_HOLD             = 12
...
def _refresh_pool(context):
    etfs = _etf_list_for_refresh(
        g.regime,
        g.BEAR_UNIVERSE_MODE,
        CANDIDATE_ETFS,
        DEFENSIVE_ETF_POOL,
    )
    if g.regime == 'BEAR' and g.BEAR_UNIVERSE_MODE == 'ETF_DEFENSIVE':
        stocks = []
    else:
        stocks = list(set(
...
def _execute_grid(context):
    ...
    layer_for_cap = (
        eff_layer
        if (g.regime == 'BEAR' and g.BEAR_GRID_MODE == 'CAP_LAYER')
    ...
    if g.regime == 'BEAR' and g.BEAR_GRID_MODE == 'NO_NET_ADD':
