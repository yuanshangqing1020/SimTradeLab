# -*- coding: utf-8 -*-
"""
多标的自适应网格策略 v2.0（JoinQuant 版）

移植自 SimTradeLab strategies/grid_multi_asset_v2/backtest.py / template.py
Walk-Forward 调参（2019-2024）最优参数 Trial 29（2026-05-10）：
  综合得分（WF）≈ -0.3457 ；Holdout（2025-01～2026-03）与 SimTradeLab 对齐：年化约 +28.94%，夏普约 1.43，最大回撤约 -12.35%

v2 相对 v1：
  - 沪深300：MA120/MA250 判断 BULL/NEUTRAL/BEAR，三档总仓比例（BULL_RATIO/NEUTRAL_RATIO/BEAR_RATIO）
  - 单标的权重 water-filling 截断；网格市值按 invested_ratio × 总资产（上限 TARGET_CAPITAL）

PTrade → JoinQuant 映射与 v1 相同，参见 strategies_jq/grid_multi_asset/v1/strategy.py 文件头说明。
"""
import numpy as np
import pandas as pd

# ── ETF 候选池（JoinQuant 代码）───────────────────────────────────────────── #
CANDIDATE_ETFS = [
...
def initialize(context):
    ...
    # Walk-Forward Trial 29（与 SimTradeLab best_params_20260510_011341.json 一致）
    g.MAX_HOLD             = 10
    g.GRID_STEP_VOL_FACTOR = 0.60
    g.GRID_STEP_MIN        = 0.02
    g.GRID_STEP_MAX        = 0.03
    g.GRID_MAX_LAYER       = 2
    g.LAYER_FRACTION       = 0.08
    g.VOL_WEIGHT           = 0.50
    g.REBALANCE_FREQ       = 20
    g.BULL_RATIO           = 0.70
    g.NEUTRAL_RATIO        = 0.60
    g.BEAR_RATIO           = 0.25
