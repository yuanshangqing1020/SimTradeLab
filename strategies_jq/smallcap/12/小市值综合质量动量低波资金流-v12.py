# -*- coding: utf-8 -*-
# 小市值综合策略 v12（SimTradeLab/strategies_jq/smallcap/12）
# 融合目录内成熟做法与国别研究中常见的多因子思路：
# - 池：399101；国九盈利/收入；次新/ST/涨跌停过滤；季节空仓。
# - 选：总市值最小的一档候选内，按 ROE、20 日动量、20 日波动率与主力资金流综合打分。
# - 仓：399101 相对 MA10 动态 3~6 只；周三先卖后买；涨停持有；止损/止盈/广度风控。
#
# 请在聚宽回测中校验参数与接口字段。
...
def _date_in_pass_period(d):
    """用月/日判断；g.pass_periods 每项须为同一个月内 MM-DD～MM-DD（与现有配置一致）。"""
    m, day = d.month, d.day
...
def get_stock_list(context):
    ...
    for c in codes:
        ...
        # diff 长度为 20，分母取与之一一对应的昨收：closes[-21]..closes[-2]
