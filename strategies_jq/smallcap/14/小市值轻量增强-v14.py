# -*- coding: utf-8 -*-
# 小市值轻量增强 v14（相对 v13：去掉「组合熔断 + 每日强平货基 + MA 减仓 + ATR」等易伤收益的风控叠层）
#
# 思路：
# - 主干与 v12 一致：399101、国九、季节空仓、MA10 动态 3～6 只、周三先卖后买、涨停持有、
#   质量 + 20 日动量 − 波动 + 资金流打分、涨跌停极端过滤、个股止损 + 翻倍止盈 + 广度止损。
# - 轻量增强（尽量不牺牲趋势段的仓位）：
#   1) 近 20 日日均成交额（收盘价×成交量）下限，剔除极端小流动性（滑点与闪崩更可控）。
#   2) 周买入日对目标池等权分配总权益的 98%（相对 v12「只把现金分给新股」更贴近目标权重）。
# - 参数在 initialize 中可调；回测请在聚宽内校验。
...
def initialize(context):
    ...
    g.min_daily_amount = 2e6
    g.liquidity_relax_min_codes = 25
...
def _liquidity_ok_codes(context, codes):
    """近 20 日（截至昨收）日均成交额；持仓股始终保留。过滤后过少则放弃流动性过滤。"""
    if not codes:
        return []
    ...
    try:
        ...
        for c in codes:
            ...
            amt = (cl[-21:-1] * vol[-21:-1]).mean()
            if amt >= g.min_daily_amount:
                ok.add(c)
        filtered = [c for c in codes if c in ok]
        if len(filtered) < g.liquidity_relax_min_codes:
            return list(codes)
        return filtered
    except Exception:
        return list(codes)
...
def get_stock_list(context):
    ...
    codes = _liquidity_ok_codes(context, codes)
...
def weekly_buy_task(context):
    ...
    if not g.trading_signal or not g.target_list:
        return
    rebalance_equal(context, g.target_list)


def rebalance_equal(context, target_list):
