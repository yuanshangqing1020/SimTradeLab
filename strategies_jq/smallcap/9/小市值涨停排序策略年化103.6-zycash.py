# 克隆自聚宽文章：https://www.joinquant.com/post/65293
# 标题：小市值涨停排序策略｜年化103.6%
# 作者：zycash
...
def get_stock_list(context):
    n_days = g.lookback_days
    cand_count = g.candidate_count
    
    log.info(f"开始选股逻辑：在市值最小的 {cand_count} 只股票中，优选过去 {n_days} 天涨停次数最多的")

    # 1. 获取基础股票池 (中小综指) 并进行基础过滤 (ST, 停牌, 次新等)
    MKT_index = '399101.XSHE'
    initial_list = get_index_stocks(MKT_index)
    initial_list = filter_stocks(context, initial_list)
    
    if not initial_list:
        log.warning("基础股票池为空")
        return []

    # 2. 【核心步骤】获取市值数据并进行初筛（构建小市值安全垫）
    # 我们只关心市值最小的那一批，这样即便选错了，回撤也相对可控
    q = query(
        valuation.code,
    ...
    ).limit(cand_count)            # 只取前 g.candidate_count 只
    
    df_cap = get_fundamentals(q)
    
    if df_cap.empty:
        log.warning("获取市值数据失败")
        return []
    
    # 此时的 target_pool 已经是全市场市值最小的一批股票了
    target_pool = list(df_cap['code'])

    # 3. 获取这批小市值股票的近期行情数据
    df_price = get_price(target_pool, end_date=context.previous_date, frequency='daily', 
                         fields=['close', 'high_limit'], count=n_days, panel=False)
    ...
    # 4. 统计涨停次数
    # 标记涨停 (收盘价 >= 涨停价 - 0.01)
    df_price['is_limit_up'] = df_price['close'] >= (df_price['high_limit'] - 0.01)
    
    # 按股票代码分组统计涨停总数
    limit_up_counts = df_price.groupby('code')['is_limit_up'].sum()
    
    # 将 Series 转为 DataFrame，列名为 limit_up_num
    df_counts = limit_up_counts.to_frame(name='limit_up_num')

    # 5. 【核心步骤】合并市值和涨停次数，进行双重排序
    # 将 df_cap 的索引设为 code，方便 join
    df_cap = df_cap.set_index('code')
    
    # 合并数据：final_df 现在同时包含 'market_cap' 和 'limit_up_num'
    final_df = df_cap.join(df_counts)
    
    # 填充 NaN：如果没有行情数据导致涨停次数为空，填充为0
    final_df['limit_up_num'].fillna(0, inplace=True)

    # 排序逻辑 (非常重要)：
    # 第一优先级：limit_up_num (涨停次数) -> 降序 (False) -> 优先选活跃的
    # 第二优先级：market_cap (总市值)     -> 升序 (True)  -> 次数一样时，选盘子更小的
    final_df = final_df.sort_values(by=['limit_up_num', 'market_cap'], ascending=[False, True])

    # (可选) 过滤掉一次都没涨停过的，如果你只想做强势股，可以取消下面这行的注释
    final_df = final_df[final_df['limit_up_num'] > 0]

    # 6. 截取最终持仓列表
    # g.stock_num 是你在 initialize 中设置的持仓数量
    final_list = list(final_df.index[:g.stock_num])
    
    # 打印前几名选股结果，用于回测日志检查
    log.info(f"选股完成，前3名详情:\n{final_df.head(3)}")
