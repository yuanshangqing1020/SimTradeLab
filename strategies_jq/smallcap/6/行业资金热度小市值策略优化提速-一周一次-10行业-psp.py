# 克隆自聚宽文章：https://www.joinquant.com/post/64291
# 标题：[年化125%回撤9%] 行业资金热度小市值策略优化提速
# 作者：psp
...
def initialize(context):
    ...
    run_weekly(before_market_open, 2, '9:01')  # 每周二10:30执行卖出操作
...
def get_stock_list(context):
    ...
    if not g.top_sectors:
        ...
        log.info(f"从100只股票中筛选出属于资金流入5-15位行业的股票数量: {len(industry_filtered_stocks)}")
    
    # 新增过滤条件：剔除股价低于3元的股票
    industry_filtered_stocks = filter_low_price_stock(context, industry_filtered_stocks)
    
    # 新增过滤条件：剔除过去5天内出现过涨停的股票
    industry_filtered_stocks = filter_recent_limitup_stock(context, industry_filtered_stocks)
...
def filter_low_price_stock(context, stock_list):
    if not stock_list:
        return []
    current_prices = history(1, unit='1d', field='close', security_list=stock_list)
    return [
        stock for stock in stock_list
        if current_prices[stock][-1] >= 3.0
    ]


# 新增：过滤过去5天内出现过涨停的股票
def filter_recent_limitup_stock(context, stock_list):
    if not stock_list:
        return []
    ...
    for stock in stock_list:
        ...
        # 检查过去5个交易日是否有涨停
        if not stock_data.empty and (stock_data['close'] == stock_data['high_limit']).any():
            exclude_stocks.add(stock)
            log.info(f"股票[{stock}]因过去5天内出现过涨停被过滤")
...
def before_market_open(context):
    ...
    try:
        ...
        # 3. 批量获取股票行业信息（优化：使用批量查询）
        # 尝试一次性获取所有股票的行业信息
        try:
            # 先尝试批量获取
            all_industries = get_industry(index_stocks, date=current_date)
            
            # 如果批量获取成功，处理数据
            if isinstance(all_industries, dict):
                stock_sector_map = {}
                for stock, info in all_industries.items():
                    if 'sw_l2' in info:
                        sector_code = info['sw_l2'].get('industry_code')
                        if sector_code and sector_code in sw_code_to_name:
                            stock_sector_map[stock] = sw_code_to_name[sector_code]
                
                log.debug(f"批量获取行业信息成功，有效映射数量: {len(stock_sector_map)}")
            else:
                # 如果批量获取失败，回退到逐个获取
                raise Exception("批量获取失败")
                
        except Exception as e:
            # 批量获取失败，使用原来的逐个获取方式（但限制频率）
            log.debug(f"批量获取行业信息失败，使用逐个获取方式: {str(e)}")
            stock_sector_map = {}
            error_count = 0
            ...
            for i, stock in enumerate(index_stocks):
                ...
                try:
                    ...
                    if error_count % 100 == 0:
                        log.warning(f"处理股票[{stock}]行业信息出错({error_count}次)")
                    continue
            
            log.debug(f"逐个获取完成，有效股票-行业映射数量: {len(stock_sector_map)}/{total_stocks}")
            ...
            for i in range(0, len(all_stocks), batch_size):
                ...
                try:
                    ...
                    if isinstance(batch_df, pd.DataFrame) and not batch_df.empty:
                        ...
                        log.debug(f"资金流数据获取进度: {batch_index}/{total_batches}")
                except Exception as batch_e:
                    log.warning(f"第{batch_index}批资金流数据获取失败: {str(batch_e)}")
                    continue
            
            if money_flow_batches:
                money_flow_df = pd.concat(money_flow_batches, ignore_index=True)
                log.debug(f"资金流数据合并完成，共{len(money_flow_df)}条记录")
        ...
        # 7. 取前10位的行业（原来是取第5-15位，这里保持一致性）
        if len(sector_fund_flow) >= 10:
            g.top_sectors = sector_fund_flow.index[:10].tolist()
        else:
            g.top_sectors = list(sector_fund_flow.index)
        
        log.info(f"====== 资金流入前{len(g.top_sectors)}位申万二级行业 ======")
