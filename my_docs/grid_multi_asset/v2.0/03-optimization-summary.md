...
```bash
# 顶部配置：strategy_name / start_date / end_date（与 v1 全长横比时可用 2019-01-01～2026-04-20）/ initial_capital
...
> **含优化区间内全部牛熊结构**；截止日期与 [v1.0 §5.2](../v1.0/03-optimization-summary.md) **对齐**，便于 **同窗口** 横比。**与 §5.1** 并举：Holdout 强弱不直接外推全长年化或可承受回撤。

SimTradeLab 实测（`run_backtest.py`：`strategy_name='grid_multi_asset_v2'`，`end_date='2026-04-20'`，初始资金 **50** 万；**2026-05-10** 日志，`stats/backtest_190101_260420_*.log`）。
...
| 索提诺比率 | 0.608 |
| 卡玛比率 | 0.151 |
| vs 000300.SS | 超额收益 **-9.57%** \| Alpha **+2.22%** \| Beta **0.545** |
| 盈利天数 | 890 / 1768（**50.4%**） |
| 盈亏比 | 1.07 |
| 持仓规模 | 约 **9.8** 只（最大约 **11** 只） |
| 期末资产 | 约 **50 万 → 75.3 万** |

**与同窗口 v1（Trial 53）对照：** v1 全长（同日截止）总收益 **+57.67%**、年化 **+6.70%**、最大回撤 **-59.76%**、夏普 **0.389**，相对沪深300超额 **-2.54%**（见 [v1.0 §5.2](../v1.0/03-optimization-summary.md)）。v2 **最深回撤显著更浅**，夏普略高，但总收益 / 年化 / **跑输基准的幅度大于 v1**；取舍取决于更重视长跑尾部回撤还是净值与超额。

**Holdout §5.1 vs 本节 §5.2（同一策略、不同跨度）：**

| 维度 | Holdout（2025～2026-03） | 全样本截止 2026-04-20 |
|------|--------------------------|----------------------|
| 年化 | **高**（+28.94%） | **低**（+6.01%） |
| 最大回撤 | **-12.35%** | **-39.83%** |
| 信息比 | 正（约 **1.205**） | **-0.063** |

因此：**regime 降仓有助于全长最大回撤画像（相对 v1 同截止窗口也有优势），但 long-only 网格仍可能长期跑输指数**；须同时阅读 §5.1 与 §5.2。
...
3. **全长回测（与 v1 同截止日 2026-04-20）：** 年化约 **+6.01%**，最大回撤 **-39.83%**，夏普 **0.449**，Beta **0.545**；与 v1 同窗相比 **回撤缓和、夏普略好**，但总收益 / 年化 / 相对 300 **超额弱于 v1**（参见 §5.2 表格下对照段落）。整体仍与 「近段 Holdout」 不可等同。
