# strategies/grid_multi_asset_v4/optimization/post_select_eligible.py
# -*- coding: utf-8 -*-
"""Walk-Forward 结束后对候选参数跑三重门禁的薄封装。

用法（在 SimTradeLab 根目录）::

    python strategies/grid_multi_asset_v4/optimization/post_select_eligible.py \\
        strategies/grid_multi_asset_v4/optimization/results/best_params_xxx.json
...
def main() -> None:
    ...
    spec = importlib.util.spec_from_file_location('_v4_gate', str(opt_dir / 'gate_eval.py'))
    if spec is None or spec.loader is None:
        raise RuntimeError('gate_eval load failed')
    gate = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(gate)
    mapping = gate.load_v4_custom_mapping()
