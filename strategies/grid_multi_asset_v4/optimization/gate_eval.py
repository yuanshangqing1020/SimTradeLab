# strategies/grid_multi_asset_v4/optimization/gate_eval.py
# -*- coding: utf-8 -*-
"""
v4 三重门禁：FULL / RECENT / BEAR 分段回测与 I∧II∧III 判定。
...
用法示例:
    ...
    python strategies/grid_multi_asset_v4/optimization/gate_eval.py \\
        --params-json strategies/grid_multi_asset_v4/optimization/results/best_params_xxx.json
"""
import argparse
import json
...
def load_v4_custom_mapping() -> dict[str, str]:
    """与同目录 optimize_params.V4_CUSTOM_MAPPING 一致（文件加载避免包路径依赖）。"""
    import importlib.util

    opt = Path(__file__).resolve().parent / 'optimize_params.py'
    spec = importlib.util.spec_from_file_location('_grid_v4_opt_params', str(opt))
    if spec is None or spec.loader is None:
        raise ImportError('无法加载 optimize_params: {}'.format(opt))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return dict(mod.V4_CUSTOM_MAPPING)


def main() -> None:
    parser = argparse.ArgumentParser(description='v4 triple-gate evaluation')
    ...
    mapping = load_v4_custom_mapping()
